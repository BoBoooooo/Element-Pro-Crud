...
<template>
  <div v-if="show">
    <el-form label-position="top" class="form" size="mini">
      ...
      <el-form-item v-if="elementConfig.type != 'button' && elementConfig.type != 'upload' && elementConfig.type != 'text'&& elementConfig.type != 'grid' "
      ...
      <el-form-item label="显示文本"
                    v-if="Object.keys(elementConfig.options).indexOf('text')>=0">
      ...
      <el-form-item label="大小"
                    v-if="Object.keys(elementConfig.options).indexOf('size')>=0">
        宽度：<el-input size="mini" style="width: 90px;"
                  type="number"
                  v-model.number="elementConfig.options.size.width"></el-input>
        高度：<el-input size="mini" style="width: 90px;"
                  type="number"
                  v-model.number="elementConfig.options.size.height"></el-input>
      </el-form-item>
      <el-form-item label="占位内容"
                    v-if="Object.keys(elementConfig.options).indexOf('placeholder')>=0
                    && (elementConfig.type!='time' || elementConfig.type!='date')">
      ...
      <el-form-item label="tips提示" v-if="elementConfig.options.tips">
        <el-input size="mini" type="textarea"
                  v-model="elementConfig.options.tips"></el-input>
      </el-form-item>
      <el-form-item label="布局方式"
                    v-if="Object.keys(elementConfig.options).indexOf('inline')>=0">
      ...
      <el-form-item label="显示输入框"
                    v-if="Object.keys(elementConfig.options).indexOf('showInput')>=0">
        <el-switch v-model="elementConfig.options.showInput"></el-switch>
      </el-form-item>
      <el-form-item label="最小值"
                    v-if="Object.keys(elementConfig.options).indexOf('min')>=0">
        <el-input-number size="mini" v-model="elementConfig.options.min"
                         :min="0"
                         :max="100"
                         :step="1"></el-input-number>
      </el-form-item>
      <el-form-item label="最大值"
                    v-if="Object.keys(elementConfig.options).indexOf('max')>=0">
        <el-input-number size="mini" v-model="elementConfig.options.max"
                         :min="0"
                         :max="100"
                         :step="1"></el-input-number>
      </el-form-item>
      <el-form-item label="步长"
                    v-if="Object.keys(elementConfig.options).indexOf('step')>=0">
        <el-input-number size="mini" v-model="elementConfig.options.step"
                         :min="0"
                         :max="100"
                         :step="1"></el-input-number>
      </el-form-item>
      <el-form-item label="是否多选"
                    v-if="'cascader,select,treeselect'.includes(elementConfig.type)">
        <el-switch v-model="elementConfig.options.multiple"
                   @change="handleSelectMuliple"></el-switch>
      </el-form-item>
      <el-form-item label="父子结点是否不关联"
                    v-if="'cascader'.includes(elementConfig.type)">
      <el-switch v-model="elementConfig.options.checkStrictly"></el-switch>
      </el-form-item>
      <el-form-item label="分隔符"
                    v-if="elementConfig.options.separator">
        <el-input size="mini" type="input"
                  v-model="elementConfig.options.separator"></el-input>
      </el-form-item>
      <el-form-item label="是否可创建"
                    v-if="elementConfig.type=='select'">
        <el-switch v-model="elementConfig.options.allowCreate"></el-switch>
      </el-form-item>
      <el-form-item label="启用按钮样式"
                    v-if="elementConfig.type=='checkbox'">
        <el-switch v-model="elementConfig.options.buttonStyle"></el-switch>
      </el-form-item>
      <el-form-item label="允许半选"
                    v-if="Object.keys(elementConfig.options).indexOf('allowHalf')>=0">
        <el-switch v-model="elementConfig.options.allowHalf">
        </el-switch>
      </el-form-item>
      <el-form-item label="支持透明度选择"
                    v-if="Object.keys(elementConfig.options).indexOf('showAlpha')>=0">
        <el-switch v-model="elementConfig.options.showAlpha">
        </el-switch>
      </el-form-item>
      <el-form-item label="是否显示标签"
                    v-if="Object.keys(elementConfig.options).indexOf('showLabel')>=0">
        <el-switch v-model="elementConfig.options.showLabel">
        </el-switch>
      </el-form-item>
      <el-form-item label="数据来源"
                    v-if="Object.keys(elementConfig.options).indexOf('options')>=0 ||
          Object.keys(elementConfig.options).indexOf('remoteOptions')>=0">
        <el-radio-group v-model="elementConfig.options.remote"
                        size="mini"
        ...
        <template v-if="elementConfig.options.remote=='dict'">
          <el-select size="mini" v-model="elementConfig.options.dictType"
                     placeholder="字典类型"
                     filterable
                     style="width: 100%">
        ...
        <template v-else-if="elementConfig.options.remote=='custom'">
          <div>
            ...
            <el-input size="mini"
                      style=""
                      v-model="elementConfig.options.remoteFunc">
              <template slot="prepend">方法名</template>
            </el-input>
            <el-input size="mini"
                      style=""
                      v-model="elementConfig.options.props.value">
              <template slot="prepend">值</template>
            </el-input>
            <el-input size="mini"
                      style=""
                      v-model="elementConfig.options.props.label">
              <template slot="prepend">标签</template>
            </el-input>
            <el-input size="mini"
                      style=""
                      v-model="elementConfig.options.props.rightLabel">
        ...
        <template v-else>
          <template v-if="elementConfig.type=='radio' || (elementConfig.type=='select'&&!elementConfig.options.multiple)">
            <el-radio-group v-model="elementConfig.options.defaultValue">
              <Draggable tag="ul"
                         :list="elementConfig.options.options"
                         v-bind="getDraggableOptions()">
                <li v-for="(item, index) in elementConfig.options.options"
                    :key="index">
                  ...
                  <el-button size="mini" @click="handleOptionsRemove(index)"
                             circle
                             plain
                             type="danger"
                             icon="el-icon-minus"
      ...
      <el-form-item label="默认值"
                    v-if="Object.keys(elementConfig.options).indexOf('defaultValue')>=0
                    && (elementConfig.type == 'textarea' || elementConfig.type == 'input'
                    || elementConfig.type=='rate' || elementConfig.type=='color'
                    || elementConfig.type=='switch'||elementConfig.type == 'date' ||elementConfig.type == 'select' ||elementConfig.type == 'html')">
        <el-input size="mini" v-if="elementConfig.type=='textarea'"
                  type="textarea"
                  :rows="5"
                  v-model="elementConfig.options.defaultValue"></el-input>
        <el-input size="mini" v-if="elementConfig.type=='input' || elementConfig.type == 'select'"
                  v-model="elementConfig.options.defaultValue"></el-input>
        ...
        <el-rate v-if="elementConfig.type == 'rate'"
                 style="display:inline-block;vertical-align: middle;"
                 :max="elementConfig.options.max"
                 :allow-half="elementConfig.options.allowHalf"
                 v-model="elementConfig.options.defaultValue"></el-rate>
        <el-button size="mini" type="text"
                   v-if="elementConfig.type == 'rate'"
                   style="display:inline-block;vertical-align: middle;margin-left: 10px;"
                   @click="elementConfig.options.defaultValue=0">清空</el-button>
        <el-color-picker v-if="elementConfig.type == 'color'"
                         v-model="elementConfig.options.defaultValue"
                         :show-alpha="elementConfig.options.showAlpha"></el-color-picker>
      ...
      <template v-if="elementConfig.type == 'time' || elementConfig.type == 'date'">
        ...
        <el-form-item label="是否为范围选择"
                      v-if="elementConfig.type == 'time'">
          <el-switch v-model="elementConfig.options.isRange">
          </el-switch>
        </el-form-item>
        <!-- 日期选择器选择范围 暂时只提供大于当前时间 小于当前时间 -->
        <el-form-item label="选择范围"
                      v-if="elementConfig.type == 'date'">
          <el-select size="mini" v-model="elementConfig.options.range"
                     clearable
        ...
        <el-form-item label="是否获取时间戳"
                      v-if="elementConfig.type == 'date'">
          <el-switch v-model="elementConfig.options.timestamp">
          </el-switch>
        </el-form-item>
        <el-form-item label="占位内容"
                      v-if="(!elementConfig.options.isRange && elementConfig.type == 'time')
                      || (elementConfig.type != 'time' && elementConfig.options.type != 'datetimerange'
                      && elementConfig.options.type != 'daterange')">
          <el-input size="mini" v-model="elementConfig.options.placeholder"></el-input>
        </el-form-item>
        <el-form-item label="开始时间占位内容"
                      v-if="(elementConfig.options.isRange) || elementConfig.options.type=='datetimerange'
                      || elementConfig.options.type=='daterange'">
        ...
        <el-form-item label="默认值"
                      v-if="elementConfig.type=='time' && Object.keys(elementConfig.options).indexOf('isRange')>=0">
      ...
      <template v-if="elementConfig.type == 'grid'">
        ...
        <el-form-item label="列配置项">
          <Draggable tag="ul"
                     :list="elementConfig.columns"
                     v-bind="getDraggableOptions()">
            <li v-for="(item, index) in elementConfig.columns"
                :key="index">
              <i class="drag-item"
                 style="font-size: 16px;margin: 0 5px;cursor: move;">
                <Icon name="bars"></Icon>
              ...
              <!-- <el-input size="mini" placeholder="栅格值"
                        style="width: 100px;"
                        type="number"
      ...
      <template v-if="elementConfig.type !== 'grid'">
        <el-form-item v-if="elementConfig.type === 'upload'"
                      label="关联字段名">
          <el-input size="mini" v-model="elementConfig.options.resourceId"></el-input>
        </el-form-item>
        <el-form-item v-if="elementConfig.type === 'upload'"
                      label="附件类型">
          <el-input size="mini" v-model="elementConfig.options.fileType"
                    placeholder="关联多个附件的情况下需要填写"></el-input>
        </el-form-item>
        <el-form-item label="操作属性"
                      v-if=" elementConfig.type !== 'table' && elementConfig.type !== 'blank' && elementConfig.type !== 'text' && elementConfig.type!== 'button'">
          <el-checkbox v-model="elementConfig.options.readonly"
                       v-if="Object.keys(elementConfig.options).indexOf('readonly')>=0">完全只读</el-checkbox>
          <el-checkbox v-model="elementConfig.options.disabled"
          ...
          <el-checkbox v-model="elementConfig.options.arrowControl"
                       v-if="Object.keys(elementConfig.options).indexOf('arrowControl')>=0">
            使用箭头进行时间选择
          </el-checkbox>
        </el-form-item>
        <el-form-item v-if="elementConfig.type != 'button' && elementConfig.type != 'table' && elementConfig.type !== 'blank' && elementConfig.type != 'upload' && elementConfig.type != 'text'"
                      label="校验">
          <div>
            <el-checkbox v-model="elementConfig.options.required">必填</el-checkbox>
          </div>
          <el-select v-if="Object.keys(elementConfig.options).indexOf('dataType')>=0"
                     v-model="elementConfig.options.dataType"
                     size="mini">
        ...
        <el-form-item label="预设查询参数" v-if="elementConfig.options.tableParams !==  undefined">
          <el-input size="mini" v-model="elementConfig.options.tableParams"
                    placeholder="例如:子表字段1,主表字段1|子表字段2,主表字段2"></el-input>
        </el-form-item>
        ...
        <el-form-item label="界面元素控制" v-if="elementConfig.options.visibleList !==  undefined">
          <el-switch v-model="elementConfig.options.visibleList.actionColumn"
                      inactive-text="是否显示操作列"
                      style="margin-right:10px"></el-switch>
          <el-switch v-model="elementConfig.options.visibleList.btnAdd"
                      inactive-text="新增按钮"
                      style="margin-right:10px"></el-switch>
          <el-switch v-model="elementConfig.options.visibleList.actionColumnBtnEdit"
                      inactive-text="编辑按钮"></el-switch>
          <el-switch v-model="elementConfig.options.visibleList.actionColumnBtnDel"
                      inactive-text="删除按钮"
                      style="margin-right:10px"></el-switch>
          <el-switch v-model="elementConfig.options.visibleList.actionColumnBtnDetail"
                      inactive-text="查看按钮"></el-switch>
          <el-switch v-model="elementConfig.options.visibleList.tableTitle"
                      inactive-text="表格标题"
                      style="margin-right:10px"></el-switch>
          <el-switch v-model="elementConfig.options.visibleList.searchForm"
                      inactive-text="查询区域"></el-switch>
        </el-form-item>
        <template v-if="elementConfig.type === 'treeselect'">
          <el-form-item label="最大高度">
            <el-input size="mini" v-model="elementConfig.options.maxHeight"
